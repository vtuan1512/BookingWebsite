...
if (isset($_POST['add_room'])) {
    ...
    $ql = "INSERT INTO `rooms`( `name`, `area`, `hotel`,`hotel_id` ,`price`, `quantity`, `adult`, `children`, `description`) VALUES (?,?,?,?,?,?,?,?,?)";
    $values = [$frm_data['name'], $frm_data['area'], $frm_data['hotel'],$frm_data['hotel_id'] ,$frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'], $frm_data['desc']];
    if (insert($ql, $values, 'sisiiiis')) {
        $flag = 1;
    }
...
if (isset($_SESSION['managerId'])) {
    ...
    $query_manager = "SELECT * FROM account_manager WHERE account_number=?";
    $value_manager = [$manager_id];
    $result_manager = select($query_manager, $value_manager, 'i');
...
if (isset($_SESSION['hotelId'])) {
    ...
    $query_hotel = "SELECT * FROM hotel WHERE id=?";
    $value_hotel = [$hotel_id];
    ...
    if ($result_hotel && mysqli_num_rows($result_hotel) > 0) {
        ...
        echo "Không có thông tin khách sạn nào được tìm thấy!";
    }
}
if ($manager_id !== null) {
    $res_manager_hotel = select("SELECT `hotel_id` FROM `manager_hotel` WHERE `manage_id`=?", [$manager_id], 'i');

    if ($res_manager_hotel && mysqli_num_rows($res_manager_hotel) > 0) {
        $row_manager_hotel = mysqli_fetch_assoc($res_manager_hotel);
        $user_hotel_id = $row_manager_hotel['hotel_id'];
        if (isset($_POST['get_all_rooms'])) {
            $res = select("SELECT * FROM `rooms` WHERE `removed`=? AND `hotel_id`=?", [0, $user_hotel_id], 'ii');
            $i = 1;
        ...
        echo "User is not authorized to manage any hotels.";
    }
}
...
if (isset($_POST['edit_room'])) {
    ...
    $q1 = "UPDATE `rooms` SET `name`=?,`area`=?,`hotel`=?,`hotel_id`=?,`price`=?,`quantity`=?,
        `adult`=?,`children`=?,`description`=? WHERE `id`=?";

    $values = [$frm_data['name'], $frm_data['area'], $frm_data['hotel'],$frm_data['hotel_id'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'], $frm_data['desc'], $frm_data['room_id']];

    if (update($q1, $values, 'sisiiiiisi')) {
        $flag = 1;
    }
