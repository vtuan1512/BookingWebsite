...
if (isset($_POST['get_bookings'])) 
{
    ...
    // Lấy hotel_id của người quản lý từ session
    $manager_hotel_id = $_SESSION['hotelId'] ?? null;

    // Kiểm tra xem hotel_id của người quản lý có tồn tại không
    if (!$manager_hotel_id) {
    ...
    AND (bo.booking_status= ? AND bo.arrival = ? AND bo.hotel_id = ?) ORDER BY bo.booking_id ASC";

    $res = select($query,["%$frm_data[search]%","%$frm_data[search]%","%$frm_data[search]%","booked",0, $manager_hotel_id],'sssssi');
    $i = 1;
    $table_data = "";
